...
public class DykstraMatrix<T> implements IGraph<T> {
	...
	public DykstraMatrix(Node<T> node) {
        this.graph = new GraphAMatrix<T>(node);
	...
	private void addNachbarnZuRand(List<Wegpunkt<T>> zielListe, Node<T> nodes) {
        List<Node<T>> nachbarn = getNachbarknoten(nodes);
        for(Node<T> node : nachbarn){
            Wegpunkt<T> tmp = getWegpunkt(this.alleWegpunkte, node);
            if(!(containsKnoten(zielListe, node) || tmp.isMarkiert)){
                zielListe.add(tmp);
            }
        }
	}
